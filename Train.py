...
def Img_learn(adata, image, lr=0.001, epochs=800, device = 'mps'):
    ...
    for epoch in tqdm(range(epochs)):
        model_img.train()
        optim_img.zero_grad()
        img_rec, img_feat = model_img(img)
    ...
    with torch.no_grad():
        model_img.eval()
        img_rec_final, img_feat_final = model_img(img)

    adata.obsm['img_feat'] = img_feat_final.detach().cpu().numpy().reshape(img_feat_final.shape[0], img_feat_final.shape[1], -1)


class Hybrid_train():
    def __init__(self, adata, device='mps', learning_rate=0.001, weight_decay=0.00, epochs=600, dim_output=32,
                 random_seed=619, alpha=10, beta=1, theta=0.1):
        self.adata = adata
        self.device = device
...
class proST():
    def __init__(self, adata, random_seed=619, device='mps'):
        self.adata = adata
        self.random_seed = random_seed
        self.device = device
    ...
    def train(self):
        ...
        emb_gene, rec_gene = model.train_gene()
        img_score, rec_img = model.train_img()
